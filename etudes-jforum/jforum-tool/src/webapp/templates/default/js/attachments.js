...
<#if attachmentsEnabled>
	...
	<#--<<<Murthy : 11/05/2008-->
	function validateAttachDescription() {
		
		var alltxtareas = document.getElementsByTagName("textarea");
		
		if (alltxtareas.length == 1)
			return true;
		...
		for (var i = 1; i < alltxtareas.length; i++) 
		{
			...
			if (name.match("comment_") != null)
			{
				if (ele.value.length > 255 )
				{
					alert("${I18n.getMessage("PostForm.maxAttachmentDescriptionLength")}");
					ele.focus();
					return false;
				}
			}
...
<#if attachments?exists>
	...
	function editAttachments()
	{
		...
		<#list attachments as a>
			...
			<#if a.info.comment??>
				<#assign s = a.info.comment>
			<#else>
				<#assign s = "">
			</#if>
			attach_${a.id}["description"] = "${s?replace('\\r\n*', ' ', 'r')}";
