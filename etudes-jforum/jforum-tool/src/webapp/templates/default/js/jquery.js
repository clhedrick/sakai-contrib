 ...
 *
 * $Date: 2013-04-30 15:03:29 -0400 (Tue, 30 Apr 2013) $
 * $Rev: 83559 $
 */
( function() {
	...
	var jQuery = window.jQuery = function(selector, context) {
		return this instanceof jQuery ? this.init(selector, context)
	...
	jQuery.fn = jQuery.prototype = {
		init : function(selector, context) {
			...
			if (typeof selector == "string") {
				...
				return new jQuery(document)[jQuery.fn.ready ? "ready" : "load"]
						(selector);
			return this
					.setArray(selector.constructor == Array
							&& selector
		...
		attr : function(key, value, type) {
			...
			return this.each( function(index) {
				for ( var prop in obj)
					jQuery.attr(type ? this.style : this, prop, jQuery.prop(
							this, obj[prop], type, index, prop));
			});
		},
		...
		text : function(e) {
			...
			jQuery.each(e || this, function() {
				jQuery.each(this.childNodes, function() {
					if (this.nodeType != 8)
						t += this.nodeType != 1 ? this.nodeValue : jQuery.fn
								.text( [ this ]);
				});
			});
			return t;
		...
		find : function(t) {
			...
			return this
					.pushStack(/[^+>] [^+>]/.test(t) || t.indexOf("..") > -1 ? jQuery
							.unique(data)
							: data);
		},
		clone : function(events) {
			var ret = this.map( function() {
				return this.outerHTML ? jQuery(this.outerHTML)[0] : this
						.cloneNode(true);
			});
		...
		filter : function(t) {
			return this.pushStack(jQuery.isFunction(t)
		...
		not : function(t) {
			return this.pushStack(t.constructor == String
					...
					|| jQuery.grep(this, function(a) {
						return (t.constructor == Array || t.jquery) ? jQuery
								.inArray(a, t) < 0 : a != t;
					}));
		},
		add : function(t) {
			return this.pushStack(jQuery.merge(this.get(),
					t.constructor == String ? jQuery(t).get()
							: t.length != undefined
									&& (!t.nodeName || jQuery.nodeName(t,
											"form")) ? t : [ t ]));
		},
		is : function(expr) {
				...
				return this
						.each( function() {
							if (val.constructor == Array
								...
								this.checked = (jQuery.inArray(this.value, val) >= 0 || jQuery
										.inArray(this.name, val) >= 0);
							else if (jQuery.nodeName(this, "select")) {
								var tmp = val.constructor == Array ? val
										: [ val ];
								jQuery("option", this)
										.each(
												function() {
		...
		html : function(val) {
			return val == undefined ? (this.length ? this[0].innerHTML : null)
			...
			.extend( {
				...
				prop : function(elem, value, type, index, prop) {
					...
					return value && value.constructor == Number
							&& type == "curCSS" && !exclude.test(prop) ? value
							+ "px" : value;
				},
				className : {
					add : function(elem, c) {
					...
					remove : function(elem, c) {
						elem.className = c != undefined ? jQuery.grep(
								elem.className.split(/\s+/), function(cur) {
									return !jQuery.className.has(c, cur);
								}).join(" ") : "";
					},
					has : function(t, c) {
						return jQuery.inArray(c, (t.className || t).toString()
								.split(/\s+/)) > -1;
					}
				},
				swap : function(e, o, f) {
				...
				css : function(e, p) {
					if (p == "height" || p == "width") {
						var old = {}, oHeight, oWidth, d = [ "Top", "Bottom",
								"Right", "Left" ];
						...
						jQuery.swap(e, old, function() {
							if (jQuery(e).is(':visible')) {
								...
								e = jQuery(e.cloneNode(true)).find(":radio")
										.removeAttr("checked").end().css( {
											visibility :"hidden",
											position :"absolute",
											display :"block",
											right :"0",
								...
								var parPos = jQuery.css(e.parentNode,
										"position")
										|| "static";
								if (parPos == "static")
											...
											arg = arg
													.replace(
															...
															function(m, all,
																	tag) {
																return tag
																		.match(/^(abbr|br|col|img|input|link|meta|param|hr|area)$/i) ? m
																		: all
																				+ "></"
																				+ tag
																				+ ">";
				...
				attr : function(elem, name, value) {
					...
					if (fix[name]) {
						...
						if (name == "opacity" && jQuery.browser.msie) {
							...
							return elem.filter ? (parseFloat(elem.filter
									.match(/opacity=([^)]*)/)[1]) / 100)
									.toString() : "";
						}
	...
	jQuery.each( [ "Height", "Width" ], function(i, name) {
		...
		jQuery.fn[n] = function(h) {
			return this[0] == window ? jQuery.browser.safari
					&& self["inner" + name]
					|| jQuery.boxModel
					&& Math.max(document.documentElement["client" + name],
							document.body["client" + name])
					|| document.body["client" + name]
					: this[0] == document ? Math.max(document.body["scroll"
							+ name], document.body["offset" + name])
							: h == undefined ? (this.length ? jQuery.css(
									this[0], n) : null) : this.css(n,
									h.constructor == String ? h : h + "px");
		};
	});
						...
						if (m[1] == ":" && m[2] == "not")
							...
							var merge = {}, tmp = [], test = /(\d*)n\+?(\d*)/
									.exec(m[3] == "even" && "2n"
											|| m[3] == "odd" && "2n+1"
											|| !/\D/.test(m[3]) && "n+" + m[3]
											|| m[3]), first = (test[1] || 1) - 0, last = test[2] - 0;
							for ( var i = 0, rl = r.length; i < rl; i++) {
	...
	jQuery.event = {
				...
				if (!type) {
					...
					if (!ret) {
						if (element.removeEventListener)
							element.removeEventListener(type, jQuery.data(
									element, "handle"), false);
						else
							element.detachEvent("on" + type, jQuery.data(
									element, "handle"));
						ret = null;
						delete events[type];
					}
		...
		trigger : function(type, data, element, donative, extra) {
			...
			if (!element) {
				...
				var val, ret, fn = jQuery.isFunction(element[type] || null), evt = !data[0]
				...
				if (fn && donative !== false && val !== false
						&& !(jQuery.nodeName(element, 'a') && type == "click")) {
					this.triggered = true;
					element[type]();
				}
		...
		fix : function(event) {
			...
			if (event.pageX == null && event.clientX != null) {
				...
				event.pageX = event.clientX
						+ (e && e.scrollLeft || b.scrollLeft || 0);
				event.pageY = event.clientY
						+ (e && e.scrollTop || b.scrollTop || 0);
			}
				...
				httpSuccess : function(r) {
					try {
						return !r.status && location.protocol == "file:"
								...
								|| r.status == 304 || jQuery.browser.safari
								&& r.status == undefined;
					} catch (e) {
					}
					return false;
	...
	jQuery.fn.extend( {
		...
		toggle : function(fn, fn2) {
			return jQuery.isFunction(fn) && jQuery.isFunction(fn2) ? this
					._toggle(fn, fn2) : fn ? this.animate( {
				height :"toggle",
				width :"toggle",
				opacity :"toggle"
	...
	jQuery.fn.offset = function() {
		...
		if (elem)
			with (jQuery.browser) {
				var absolute = jQuery.css(elem, "position") == "absolute", parent = elem.parentNode, offsetParent = elem.offsetParent, doc = elem.ownerDocument, safari2 = safari
				...
				if (elem.getBoundingClientRect) {
					...
					while (offsetParent) {
						...
						if (mozilla && /^t[d|h]$/i.test(parent.tagName)
								|| !safari2)
							border(offsetParent);
						if (safari2
								&& !absolute
								&& jQuery.css(offsetParent, "position") == "absolute")
							absolute = true;
						offsetParent = offsetParent.offsetParent;
					}
					while (parent.tagName
							&& !/^body|html$/i.test(parent.tagName)) {
						if (!/^inline|table-row.*$/i.test(jQuery.css(parent,
								"display")))
