/*********************************************************************************** 
 * $URL: https://source.sakaiproject.org/contrib/etudes/sakai-jforum/tags/2.9.11/jforum-tool/src/java/org/etudes/jforum/util/user/JForumUserUtil.java $ 
 * $Id: JForumUserUtil.java 83559 2013-04-30 19:03:29Z murthy@etudes.org $ 
 *********************************************************************************** 
 * 
 ...
 * @version $Id: JForumUserUtil.java 83559 2013-04-30 19:03:29Z murthy@etudes.org $
 */
public class JForumUserUtil {
		...
		/*Get all current users*/
			    ...
			    try
			    {
			      ...
			      while (sakUserIterator.hasNext()){
			        ...
			        try{
			        	org.sakaiproject.user.api.User user = UserDirectoryService.getUser(userId);
			        	if (!JForumUserUtil.isJForumFacilitator(userId) && !JForumUserUtil.isJForumParticipant(userId))
			        	{
			        		sakUserIterator.remove();
			        	}
			        }catch (UserNotDefinedException e){
	...
	public static void updateJFUser(User jfuser)throws Exception
	{
		try
		{
			...
			// org.sakaiproject.user.api.User sakUser =
			// UserDirectoryService.getUser(username);
			...
			if (sakUser != null)
			{
				...
				/*
				 * if sakai user first name and last name are blank then change
				 * the jforum user last name as Guest
				 */
				// first name
...
private static void updateExistingMembers(List exissiteusers) throws Exception {
	...
	public static boolean isUserActive(String sakaiUserId)
	{
		...
		if ((!jforumSecurityService.isUserActive(ToolManager.getCurrentPlacement().getContext(), sakaiUserId))
					|| (jforumSecurityService.isUserBlocked(ToolManager.getCurrentPlacement().getContext(), sakaiUserId)))
		{		
			return false;
		}
		return true;
